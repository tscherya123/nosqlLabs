...
namespace Data.MongoDAO
{
    public class ItemMongoDAO : IItemDAO
    {
        ...
        public IList<Item> GetAll()
        {
            ...
            try
            {
                var infos = infosCollection.Aggregate().Match(i => true).ToList();
                var creators = creatorsCollection.Aggregate().Match(c => true).ToList();
                var itemCreators = itemCreatorCollection.Aggregate().Match(c => true).ToList();

                items = collection.Aggregate().Match(c => true).ToList().Select(c => new Item()
                {
                    Id = c.Id,
                    Info = infos.FirstOrDefault(i => i.Id == c.Info),
