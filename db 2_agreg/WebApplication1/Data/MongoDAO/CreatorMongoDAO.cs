    ...
    public class CreatorMongoDAO : ICreatorDAO
    {
        ...
        public IList<Creator> Get(int? id, string name)
        {
            ...
            try
            {
                ...
                {
                    creators = collection.Aggregate().Match(new BsonDocument(nameof(Creator.Id), id.ToString())).ToList();
                }
                else if (id == null && name != null)
                {
                    creators = collection.Aggregate().Match(new BsonDocument(nameof(Creator.Name), name)).ToList();
                }
                else if (id != null && name != null)
                {
                    creators = collection.Aggregate()
                        .Match(new BsonDocument(nameof(Creator.Id), id.ToString()))
                        .Match(new BsonDocument(nameof(Creator.Name), name)).ToList();
                }
                else
                {
                    creators = collection.Aggregate().Match(c => true).ToList();
                }
            }
            catch (Exception ex)
            {
        ...
        public IList<Creator> GetAll()
        {
            ...
            try
            {
                creators = collection.Aggregate().Match(c => true).Project(c => c).ToList();
            }
            catch (Exception ex)
            {
                throw ex;
